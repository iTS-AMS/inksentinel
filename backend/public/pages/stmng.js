 ...
 */
export async function getFeeds(seatLabel = null) {
  const qs = seatLabel ? `?seat=${encodeURIComponent(seatLabel)}` : '';
  return apiFetch(`/api/feeds${qs}`);
}
...
/**
 * Upsert a row in camera_links so the seat-camera assignment persists.
 * Called by the Link button in index.html modal.
 */
export async function setCameraLink(seatLabel, feedId, cameraId, feedLabel) {
  return apiFetch('/api/camera-links', {
    method: 'PUT',
    body: JSON.stringify({ seat_label: seatLabel, feed_id: feedId,
                           camera_id: cameraId, feed_label: feedLabel }),
  });
}

/**
 * Clear the camera_links row for a seat (unlink).
 */
export async function removeCameraLink(seatLabel) {
  return apiFetch(`/api/camera-links/${encodeURIComponent(seatLabel)}`, {
    method: 'DELETE',
  });
}

/**
 * Get the camera_links row for a specific seat.
 * Returns { feed_id, camera_id, feed_label, status } or null.
 */
export async function getCameraLink(seatLabel) {
  try {
    const data = await apiFetch('/api/camera-links');
    const link = (data.links || []).find(l => l.seat_label === seatLabel) || null;
    return link;
  } catch (_) {
    return null;
  }
