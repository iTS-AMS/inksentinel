    ...
    case 'heartbeat':
      ...
      if (Array.isArray(msg.units)) {
        ...
        msg.units.forEach(u => {
          ...
          if (typeof u.cheating !== 'undefined')
            S.units[u.mac].cheating = !!u.cheating;
...
function buildGrid() {
  ...
  for (let row = S.gridRows; row >= 1; row--) {
    for (let col = 1; col <= S.gridCols; col++) {
      ...
      if (S.seatLinks[seatNum]) cell.dataset.feedId = S.seatLinks[seatNum];
...
function renderGrid() {
  ...
  E.seatGrid.querySelectorAll('.seat-cell').forEach(cell => {
    ...
    if (S.seatLinks[seatNum]) {
      cell.dataset.feedId = S.seatLinks[seatNum];
    } else {
      cell.removeAttribute('data-feed-id');
    }
    ...
    if (!unit) {
      cell.classList.remove('occupied', 'seat-cheating');
    ...
    if (unit.cheating) {
      cell.classList.add('seat-cheating');
    } else {
      cell.classList.remove('seat-cheating');
    }
    ...
    dot.className   = 'seat-status-dot ' + (unit.cheating ? 'cheating' : (unit.status || 'inactive'));
...
function findUnitByDeviceId(deviceId) {
  return Object.values(S.units).find(u => u.id === deviceId);
}

function findUnitByFeedId(feedId) {
  return Object.values(S.units).find(u => u.feedId === feedId);
}

function setSeatCheatingState(unit, active) {
  if (!unit) return;
  unit.cheating = !!active;
  renderGrid();
}

function updateCheatStatus(msg) {
  const active = !!(msg.cheating || msg.cheating_active || msg.alertTriggered || msg.alert);
  let unit = null;

  if (msg.mac && S.units[msg.mac]) {
    unit = S.units[msg.mac];
  } else if (msg.device_id) {
    unit = findUnitByDeviceId(msg.device_id);
  } else if (msg.seat || msg.seat_number || msg.seatNum) {
    const seat = parseInt(msg.seat || msg.seat_number || msg.seatNum, 10);
    const mac  = S.seats[seat];
    unit = mac ? S.units[mac] : null;
  } else if (msg.feed_id) {
    unit = findUnitByFeedId(msg.feed_id);
  }

  if (unit) setSeatCheatingState(unit, active);
}
